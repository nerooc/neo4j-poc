...
export const getAllRacetracks = async () => {
  const response: Promise<AxiosResponse<IRacetrack[]>> =
    axiosInstance.get(`/racetrack/`);

  return response;
};

export const createNewRacetrack = (data: IRacetrack) => {
  const response: Promise<AxiosResponse> = axiosInstance.post(
    `/racetrack/`,
    data
  );
...
export const setWinner = async (racetrackId: string, driverId: string) => {
  const response: Promise<AxiosResponse<IRacetrack>> = axiosInstance.post(
    `/racetrack/winner/${racetrackId}/${driverId}`
  );
